...
static int tmpfs_dump(struct mount_info *pm)
{
	...
	ret = cr_system_userns(fd, img_raw_fd(img), -1, "tar",
			       (char *[]){ "tar", "--create", "--gzip", "--no-unquote", "--no-wildcards",
					   ...
					   "--numeric-owner", "--xattrs", "--xattrs-include=user.*", "--directory",
					   "/proc/self/fd/0", ".", NULL },
			       0, userns_pid);
...
static int tmpfs_restore(struct mount_info *pm)
{
	...
	ret = cr_system(img_raw_fd(img), -1, -1, "tar",
			(char *[]){ "tar", "--extract", "--gzip", "--no-unquote", "--no-wildcards", "--xattrs",
				    "--xattrs-include=user.*", "--directory", service_mountpoint(pm), NULL },
			0);
	close_image(img);
