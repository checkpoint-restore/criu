...
static struct cr_statmount *statmount_for_unmounted_mount(int lfd)
{
	...
	int ret = 0, bufsize = PAGE_SIZE;
	struct cr_statmount *statmnt = NULL;

retry:
	statmnt = xmalloc(bufsize);
	if (!statmnt) {
		pr_err("could not allocate buffer for statmount\n");
		return NULL;
	}

	ret = sys_statmount(&req, statmnt, bufsize, STATMOUNT_BY_FD);
	if (ret < 0) {
		if (errno == EOVERFLOW) {
			bufsize *= 2;
			goto retry;
...
struct mount_info *mount_info_for_unmounted_mount(int lfd)
{
	...
	size_t len;
	char *options;
	struct mount_info *mnt = NULL, *cur;
	cleanup_free struct cr_statmount *statmnt = NULL;
	...
	statmnt = statmount_for_unmounted_mount(lfd);
	if (!statmnt)
		return NULL;
