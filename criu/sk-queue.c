...
int sk_queue_post_actions(void)
{
	...
	list_for_each_entry_safe(pkt, t, &packets_list, list) {
		...
		if (!ret) {
			...
			if (ue->pid != 0 && !pkt->pidfd_stale) {
				/*
				 * Look the sender up in the dumped tree, unless a
				 * pidfd already proved it reaped. Exit info is
				 * recorded by pidfs_exit(), which release_task()
				 * calls, and pidfd_prepare() refuses a struct pid
				 * with no task linkage left, so either way a stale
				 * verdict means the sender has been reaped and
				 * cannot be in the tree at all. A hit here would be
				 * the same pid number handed out again to some
				 * unrelated task, and fixing the packet up to its
				 * vpid would credit the message to a process that
				 * never sent it. An unreaped zombie is a different
				 * matter: it is still in the tree, still yields a
				 * pidfd with no exit info, and is fixed up below
				 * like any other live sender. A zero pid is not a
				 * pid to look up at all, see further down.
				 */
...
/*
 * recvmsg() installs the fds of SCM_RIGHTS and SCM_PIDFD before it reports
 * anything about the control buffer, so what it installed is ours to close
 * even when we go on to fail the dump. The kernel shortens msg_controllen to
 * what it actually wrote, so this walk only ever sees whole cmsgs.
 */
static void close_received_fds(struct msghdr *mh)
{
	struct cmsghdr *ch;
	...
	for (ch = CMSG_FIRSTHDR(mh); ch; ch = CMSG_NXTHDR(mh, ch)) {
		...
		if (ch->cmsg_level != SOL_SOCKET)
			continue;
		if (ch->cmsg_type != SCM_RIGHTS && ch->cmsg_type != SCM_PIDFD)
			continue;

		fds = (int *)CMSG_DATA(ch);
		nr = (ch->cmsg_len - CMSG_LEN(0)) / sizeof(int);
		for (i = 0; i < nr; i++) {
			/* An SCM_PIDFD payload can be a raw negative errno. */
			if (fds[i] >= 0)
				close(fds[i]);
		}
	}
	...
	for (ch = CMSG_FIRSTHDR(mh); ch; ch = CMSG_NXTHDR(mh, ch)) {
		...
		if (ch->cmsg_level == SOL_SOCKET) {
			if (ch->cmsg_len == CMSG_LEN(sizeof(struct ucred)) &&
				 ...
				 * already reaped one (-EINVAL up to v6.15,
				 * -ESRCH from v6.16, where the check was split
				 * up). The rest are about us -- a full fd
				 * table, no memory -- so bail out rather than
				 * declare a live sender dead and quietly stand
				 * in for it. From v6.17 scm_pidfd_recv() asks
				 * for PIDFD_STALE and the check is skipped
				 * altogether, so a reaped sender yields a real
				 * stale pidfd and none of this applies.
				 *
				 * -EINVAL is the looser of the two: up to v6.15
				 * it also meant "this struct pid is not a
				 * thread-group leader". A sender with
				 ...
				 * v6.16 splits that out as -ENOENT, which we
				 * bail on rather than guess.
				 */
...
static int dump_sk_queue_packet(int sock_fd, int sock_id, void *data, int size, int flags)
{
	...
	if (msg.msg_flags & MSG_CTRUNC) {
		/*
		 * Control data was truncated: the kernel dropped part of the
		 * ancillary messages. We would misread the sender's liveness
		 * from a missing SCM_PIDFD, so bail out, closing whatever fds
		 * recvmsg installed before it gave up on the buffer.
		 */
