...
static char cgroup2_sb_opts_cache[256];
static bool cgroup2_sb_opts_cached;
 ...
 * STATX_MNT_ID_UNIQUE provides. Both land in Linux 6.8 and the mnt_opts
 * field in 6.11; kdat.has_statmount tells us the syscall works at all, and
 * a missing STATMOUNT_MNT_OPTS bit in the reply makes the caller fall back
 * to mountinfo. Assumes the host cgroup2 is mounted at /sys/fs/cgroup; the
 * cgroup2_collect_sb_opts fallback covers the rare case where it is not.
 */
static int cgroup2_collect_sb_opts_statmount(char *opts, size_t size)
{
	...
	struct cr_mnt_id_req req = {
		...
		.param = STATMOUNT_SB_BASIC | STATMOUNT_MNT_OPTS,
	};
	/*
	 * statmount(2) copies the mnt_opts string into the space past the
	 * fixed struct (cr_statmount ends in a flexible str[] member), so the
	 * buffer must be larger than the struct itself.
	 */
	union {
		struct cr_statmount sm;
		char buf[sizeof(struct cr_statmount) + 4096];
	} u = {};
	int fd, exit_code = -1;
	...
	fd = open("/sys/fs/cgroup", O_PATH);
	if (fd < 0) {
		pr_perror("Unable to open /sys/fs/cgroup");
		return -1;
	}

	if (syscall(SYS_statx, fd, "", AT_EMPTY_PATH, STATX_MNT_ID_UNIQUE, &stx) < 0) {
		pr_perror("Unable to get the cgroup2 mount id");
		goto err;
	}
	if (!(stx.stx_mask & STATX_MNT_ID_UNIQUE)) {
		pr_debug("statx did not return a unique cgroup2 mount id\n");
		goto err;
	}

	req.mnt_id = stx.stx_mnt_id;

	if (sys_statmount(&req, &u.sm, sizeof(u), 0) < 0) {
		pr_perror("Unable to get cgroup2 superblock options");
		goto err;
	}
	/* The kernel silently ignores STATMOUNT_* bits it does not support. */
	if ((u.sm.mask & (STATMOUNT_SB_BASIC | STATMOUNT_MNT_OPTS)) !=
	    (STATMOUNT_SB_BASIC | STATMOUNT_MNT_OPTS)) {
		pr_debug("statmount did not return cgroup2 superblock options\n");
		goto err;
	}
	if (u.sm.sb_magic != CGROUP2_SUPER_MAGIC) {
		pr_debug("/sys/fs/cgroup is not a cgroup2 mount\n");
		goto err;
	}
...
static int cgroup2_collect_sb_opts_mountinfo(char *opts, size_t size)
{
	struct bfd f = { .fd = -1 };
	char *line;
	int exit_code = -1;
	...
	while ((line = breadline(&f)) != NULL) {
		...
		super = strrchr(line, ' ');
		if (!super) {
			pr_err("Bad cgroup2 mountinfo entry: '%s'\n", line);
			goto err;
		}
		super++;
	...
	if (exit_code)
		pr_err("Unable to find a cgroup2 mount in mountinfo\n");
err:
	bclose(&f);
	return exit_code;
}

static int cgroup2_collect_sb_opts(char *opts, size_t size)
{
	if (cgroup2_sb_opts_cached) {
		__strlcpy(opts, cgroup2_sb_opts_cache, size);
		return 0;
	}

	opts[0] = '\0';
	if (cgroup2_collect_sb_opts_statmount(opts, size)) {
		opts[0] = '\0';
		if (cgroup2_collect_sb_opts_mountinfo(opts, size))
			return -1;
	}

	__strlcpy(cgroup2_sb_opts_cache, opts, sizeof(cgroup2_sb_opts_cache));
	cgroup2_sb_opts_cached = true;
	return 0;
}
...
static int __new_open_cgroupfs(struct cg_ctl *cc)
{
	...
	if (strstartswith(cc->name, namestr)) {
		...
		if (cgroup2_collect_sb_opts(opts, sizeof(opts))) {
			pr_err("Unable to collect cgroup2 superblock options\n");
			goto err;
		}
		for (tok = strtok_r(opts, ",", &saveptr); tok; tok = strtok_r(NULL, ",", &saveptr)) {
			if (cr_fsconfig(fsfd, FSCONFIG_SET_FLAG, tok, NULL, 0)) {
				fsfd_dump_messages(fsfd);
				pr_perror("Unable to preserve cgroup2 option %s", tok);
				goto err;
			}
		}
	}
...
static int open_cgroupfs(struct cg_ctl *cc)
{
	...
	if (strstartswith(cc->name, namestr))
		...
		if (cgroup2_collect_sb_opts(mopts, sizeof(mopts))) {
			pr_err("Unable to collect cgroup2 superblock options\n");
			return -1;
		}
	} else
