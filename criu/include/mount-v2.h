...
static inline int sys_statmount(struct mnt_id_req *req, struct statmount *smbuf, size_t bufsize, unsigned long flags)
{
	return syscall(__NR_statmount, req, smbuf, bufsize, flags);
}

extern int check_mount_v2(void);
extern struct statmount *do_statmount(struct mnt_id_req *req, unsigned int flags);
