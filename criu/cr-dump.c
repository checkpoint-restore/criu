...
static int cr_dump_finish(int ret)
{
	...
	/*
	 * A pre-dump script can fail before cr_plugin_init() initializes the
	 * hook chains. The plugin list is initialized statically, so it is safe
	 * to check here: if no plugins are loaded, there are no hooks to run.
	 */
	if (!list_empty(&cr_plugin_ctl.head)) {
