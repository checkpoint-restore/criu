 ...
 * apart.
 *
 * Before pidfs every pidfd was a file on one shared anonymous inode, so its
 * inode number is the same for every process and identifies nothing. Such a
 * pidfd is reported as a failure, as is anything else not on pidfs.
 *
...
int pidfd_query_ino(int pidfd, uint64_t *ino)
{
	...
	if (fstatfs(pidfd, &stfs) < 0) {
		pr_debug("fstatfs on pidfd %d failed: %s\n", pidfd, strerror(errno));
		return -1;
	}

	if (stfs.f_type != PID_FS_MAGIC) {
		pr_debug("pidfd %d is not on pidfs, its ino is no identity\n", pidfd);
		return -1;
	}
...
static uint64_t pidfd_entry_ino(const PidfdEntry *pidfe)
{
	uint64_t ino = pidfe->ino;

	if (pidfe->has_ino_hi)
		ino |= (uint64_t)pidfe->ino_hi << 32;
...
static void pr_info_pidfd(char *action, PidfdEntry *pidfe)
{
	pr_info("%s: id %#08x flags %u NSpid %d ino %" PRIu64 "\n",
		action, pidfe->id, pidfe->flags, pidfe->nspid,
		pidfd_entry_ino(pidfe)
	);
}
...
static int dump_one_pidfd(int pidfd, u32 id, const struct fd_parms *p)
{
	 ...
	 * land on one stand-in.
	 *
	 * Where it has no ino to give, the fdinfo value is no better: before
	 * pidfs it is the one anonymous inode every pidfd shares, and keeping
	 * it would restore every dead process as a single one. Record 0, which
	 * restore takes to mean "no identity".
	 */
...
/*
 * A stand-in reproduces a signal death by raising the signal with its default
 * action restored, so the signal has to be one that actually kills. A number
 * out of range, or one the kernel ignores, stops or continues on, would leave
 * the helper alive, fall through to its _exit(1) and fail the whole restore in
 * kill_status_helper() instead.
 */
static bool sig_kills_by_default(int sig)
{
	if (sig <= 0 || sig >= NSIG)
		return false;
...
int dead_pid_add(uint64_t ino, bool has_ino, PidfsAttrEntry *attr)
{
	...
	if (has_status && WIFSIGNALED(status) && !sig_kills_by_default(WTERMSIG(status))) {
		pr_err("Dead pid exit status %#x names signal %d, which does not kill by default\n", status,
		       WTERMSIG(status));
		return -1;
	}
...
static int collect_one_pidfd(void *obj, ProtobufCMessage *msg, struct cr_img *i)
{
	 ...
	 */
	ino = pidfd_entry_ino(info->pidfe);
	dead = ino ? lookup_dead_pidfd(ino) : NULL;
	if (!dead) {
		...
		dead->dead_pid_idx = dead_pid_add(ino, ino != 0, info->pidfe->attr);
