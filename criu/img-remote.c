...
/* List of local operations pending (reads on the restore side for images
...
/* Local dump or restore servicing fd. */
int local_req_fd;
...
static inline void finish_proxy_write(struct roperation *rop)
{
	...
	if (!strncmp(rop->path, DUMP_FINISH, sizeof(DUMP_FINISH))) {
		/* TODO - couldn't we handle the DUMP_FINISH in inside
		 * handle_accept_proxy_write? */
		finish_local();
	} else {
		/* Normal image received, forward it. */
		struct roperation *rop_to_forward = new_remote_operation(
			rop->path, rop->snapshot_id, opts.ps_socket, rop->flags, false);
...
static void finish_cache_write(struct roperation *rop)
{
	...
	forwarding = false;
	event_set(epoll_fd, EPOLL_CTL_ADD, opts.ps_socket, EPOLLIN, &opts.ps_socket);
...
void accept_image_connections() {
	 ...
	 * the epoll. */
	if (restoring) {
		ret = event_set(epoll_fd, EPOLL_CTL_ADD, opts.ps_socket,
			EPOLLIN, &opts.ps_socket);
		if (ret) {
	...
	while (1) {
		...
		for (i = 0; i < n_events; i++) {
			...
			} else if (restoring && !forwarding && events[i].data.ptr == &opts.ps_socket) {
				event_set(epoll_fd, EPOLL_CTL_DEL, opts.ps_socket, 0, 0);
				handle_remote_accept(opts.ps_socket);
			} else {
				struct roperation *rop =
 ...
 * Zero means read until the socket is closed.
 */
static int64_t recv_image_async(struct roperation *op)
{
	int fd = op->fd;
