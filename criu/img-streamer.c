...
static int _img_streamer_open(char *filename)
{
	 ...
	 * via protobufs, CRIU gets a broken pipe error when trying to access
	 * to the pipe. This behavior is similar to what would happen if we
	 * were connecting criu and criu-image-streamer via a shell pipe.
	 */
