...
static ssize_t cuda_enumeration_read(struct cuda_wait *wait, int fd, void *buf, size_t len)
{
	ssize_t bytes;
	int ret;

	for (;;) {
		ret = cuda_wait_fd(wait, fd);
		if (ret)
			return ret;
...
static int cuda_enumeration_read_all(struct cuda_wait *wait, int fd, void *buf, size_t len)
{
	char *ptr = buf;

	while (len) {
		/* A partial reply must not turn the next read into an unbounded wait. */
		ssize_t bytes = cuda_enumeration_read(wait, fd, ptr, len);
...
static int cuda_open_inventory_image(bool write_image, size_t *size)
{
	...
	if (opts.stream) {
		...
		if (fd < 0) {
			ret = fd == -1 ? -EIO : fd;
			if (write_image || ret != -ENOENT)
				pr_err("Unable to open CUDA GPU inventory stream for %s: %s\n",
				       write_image ? "writing" : "reading", strerror(-ret));
			return ret;
		}
	} else {
		...
		if (fd < 0) {
			ret = -errno;
			if (write_image || ret != -ENOENT)
				pr_perror("Unable to open %s for %s", CUDA_GPU_INVENTORY_IMAGE,
					  write_image ? "writing" : "reading");
			return ret;
		}
	}
	...
	if (ret) {
		pr_err("Unable to %s CUDA GPU inventory size: %s\n",
		       write_image ? "write" : "read", strerror(-ret));
		close(fd);
		return ret;
	}
	if (!write_image)
...
static int cuda_gpu_list_init(struct cuda_gpu_list *list, unsigned int count)
{
	...
	if (!list->gpu_values || !list->gpus || !list->uuids) {
		pr_err("Unable to allocate CUDA GPU inventory for %u devices\n", count);
...
static int cuda_enumerate_gpus(struct cuda_gpu_list *list)
{
	...
	if (pipe2(pipefd, O_CLOEXEC)) {
		ret = -errno;
		pr_perror("Unable to create CUDA GPU enumeration pipe");
		return ret;
	}
	...
	if (sigprocmask(SIG_BLOCK, &mask, &saved_mask)) {
		ret = -errno;
		pr_perror("Unable to block SIGCHLD for CUDA GPU enumeration");
		close(pipefd[0]);
		close(pipefd[1]);
		return ret;
	}
	...
	if (child == 0) {
		...
		if (reply.error)
			pr_err("Unable to collect CUDA GPU inventory: %s (%d)\n",
			       strerror(-reply.error), reply.error);
		else
		...
		if (ret)
			pr_err("Unable to write CUDA GPU enumeration reply: %s\n", strerror(-ret));
		cuda_gpu_list_fini(list);
		close(pipefd[1]);
		/* Successfully sending an error reply does not mean enumeration succeeded. */
		if (ret || reply.error)
			_exit(EXIT_FAILURE);
		_exit(EXIT_SUCCESS);
	}
	ret = child < 0 ? -errno : 0;
	if (ret)
		pr_perror("Unable to fork CUDA GPU enumeration process");
	close(pipefd[1]);
	if (ret) {
		close(pipefd[0]);
		goto restore_mask;
	...
	ret = cuda_wait_init(&wait, "enumerate-gpus", child, 0, NULL, cuda_plugin_timeout);
	if (ret)
		goto reap_child;
	ret = cuda_enumeration_read_all(&wait, pipefd[0], &reply, sizeof(reply));
	if (ret) {
		pr_err("Unable to read CUDA GPU enumeration reply: %s\n", strerror(-ret));
		goto reap_child;
	}
	if (reply.error) {
		pr_err("CUDA GPU enumeration helper reported error %d\n", reply.error);
		ret = reply.error < 0 ? reply.error : -EIO;
		goto reap_child;
	}
	if (!reply.count || reply.count > CUDA_GPU_INVENTORY_MAX_GPUS) {
		pr_err("Invalid CUDA GPU enumeration count %u\n", reply.count);
		ret = -EINVAL;
		goto reap_child;
	}
	...
	ret = cuda_enumeration_read_all(&wait, pipefd[0], list->uuids, reply.count * sizeof(*list->uuids));
	if (ret) {
		pr_err("Unable to read enumerated CUDA GPU UUIDs: %s\n", strerror(-ret));
		goto reap_child;
	}
	bytes = cuda_enumeration_read(&wait, pipefd[0], &byte, sizeof(byte));
	ret = bytes < 0 ? (int)bytes : bytes ? -EINVAL : 0;
	if (ret) {
		pr_err("Invalid end of CUDA GPU enumeration reply: %s\n", strerror(-ret));
		goto reap_child;
	}
...
reap_child:
	close(pipefd[0]);
	if (!ret)
		ret = cuda_wait_child(&wait, child, 0, &status);
	if (ret) {
		if (kill(child, SIGKILL) < 0 && errno != ESRCH)
			pr_perror("Unable to kill CUDA GPU enumeration process %d", child);
		cleanup_ret = cuda_wait_init(&cleanup, "reap-enumeration", child, 0, NULL,
					     CUDA_HELPER_CLEANUP_TIMEOUT);
		if (!cleanup_ret) {
			cleanup.ignore_criu_timeout = true;
			cleanup_ret = cuda_wait_child(&cleanup, child, 0, &status);
		}
		if (cleanup_ret)
			pr_err("Unable to reap CUDA GPU enumeration process %d: %s\n", child,
			       strerror(-cleanup_ret));
		goto restore_mask;
	}
	if (!WIFEXITED(status) || WEXITSTATUS(status)) {
		pr_err("CUDA GPU enumeration process %d failed with wait status %#x\n", child, status);
		ret = -EIO;
		goto restore_mask;
	}
...
restore_mask:
	if (sigprocmask(SIG_SETMASK, &saved_mask, NULL)) {
		if (!ret)
			ret = -errno;
		pr_perror("Unable to restore signal mask after CUDA GPU enumeration");
	}
	if (ret)
		cuda_gpu_list_fini(list);
	return ret;
...
static int cuda_write_inventory(const CudaGpuInventory *inventory)
{
	...
	data = malloc(size);
	if (!data) {
		pr_err("Unable to allocate %zu bytes for CUDA GPU inventory\n", size);
		return -ENOMEM;
	}
	...
	if (ret)
		pr_err("Unable to write CUDA GPU inventory: %s\n", strerror(-ret));
	if (close(fd) && !ret) {
		ret = -errno;
		pr_perror("Unable to close CUDA GPU inventory image after writing");
	}
	free(data);
	return ret;
}
...
static int cuda_validate_inventory(const CudaGpuInventory *inventory)
{
	...
	if (!seen || !uuids) {
		pr_err("Unable to allocate CUDA GPU inventory validation buffers\n");
		ret = -ENOMEM;
		goto out;
	}
...
int cuda_gpu_inventory_restore_init(void)
{
	...
	if (close(fd) && !ret) {
		ret = -errno;
		pr_perror("Unable to close CUDA GPU inventory image after reading");
	}
	if (ret) {
		free(data);
		return ret;
...
static int cuda_build_cli_value(struct cuda_device_map *map)
{
	...
	if (__builtin_mul_overflow((size_t)map->count,
		...
		pr_err("CUDA device map is too large to format: %u pairs\n", map->count);
		return -EOVERFLOW;
	}

	map->cli_value = malloc(size);
	if (!map->cli_value) {
		pr_err("Unable to allocate CUDA device map string\n");
		return -ENOMEM;
	}
...
static int cuda_validate_unique_map_uuids(const struct cuda_device_map *map)
{
	...
	if (!uuids) {
		pr_err("Unable to allocate CUDA device map validation buffer\n");
		return -ENOMEM;
	}
...
static int cuda_build_automatic_map(struct cuda_device_map *map)
{
	...
	if (!map->pairs) {
		pr_err("Unable to allocate automatic CUDA device map\n");
		ret = -ENOMEM;
		goto out;
	}
...
int cuda_device_map_resolve(const char *value, struct cuda_device_map *map)
{
	...
	if (!map->pairs) {
		pr_err("Unable to allocate CUDA device map\n");
		return -ENOMEM;
	}
	...
	if (cuda_saved_inventory) {
		...
		if (!source_seen) {
			pr_err("Unable to allocate CUDA source GPU tracking buffer\n");
			ret = -ENOMEM;
			goto out;
		}
	}
