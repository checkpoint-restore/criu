...
int main(int argc, char **argv)
{
	...
	struct sockaddr_in6 addr1, addr2, addr3, addr;
	unsigned int addr_len;
	char recv_buffer[256], buffer[256];

	test_init(argc, argv);

	sk1 = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP);
	if (sk1 < 0) {
		pr_perror("Can't create socket");
		return 1;
	}

	sk2 = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP);
	if (sk2 < 0) {
		pr_perror("Can't create socket");
		return 1;
	}

	sk3 = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP);
	if (sk3 < 0) {
		pr_perror("Can't create socket");
		return 1;
	}
	...
	addr3.sin6_family = AF_INET6;
	addr3.sin6_port = htons(PORT + 2);
	inet_pton(AF_INET6, "::1", &addr3.sin6_addr);
	...
	if (addr_len != sizeof(struct sockaddr_in6) || memcmp(&addr1, &addr, addr_len)) {
		fail("Wrong peer");
		return 1;
	}
	...
	if (addr_len != sizeof(struct sockaddr_in6) || memcmp(&addr2, &addr, addr_len)) {
		fail("Wrong peer");
		return 1;
	}
	...
	if (addr_len != sizeof(struct sockaddr_in6) || memcmp(&addr3, &addr, addr_len)) {
		fail("Wrong peer");
		return 1;
	}
