...
enum reply_behavior {
	REPLY_NORMAL,
	REPLY_PARTIAL_HEADER,
	REPLY_PARTIAL_PAYLOAD,
	REPLY_NO_EOF,
	REPLY_DELAYED_EXIT,
	REPLY_SLOW_PARTS,
};
...
static enum reply_behavior reply_behavior;
static int reply_fd = -1;
static unsigned int reply_writes;
...
pid_t __wrap_fork(void)
{
	...
	/* Also bound injected child hangs if the production deadline regresses. */
	if (!child)
		alarm(10);
	return child;
}

static void hang_helper(void)
{
	for (;;)
		pause();
}
...
ssize_t __wrap_write(int fd, const void *buf, size_t count)
{
	if (getpid() != parent_pid) {
		reply_fd = fd;
		reply_writes++;
		if ((reply_behavior == REPLY_PARTIAL_HEADER && reply_writes == 1) ||
		    (reply_behavior == REPLY_PARTIAL_PAYLOAD && reply_writes == 2)) {
			assert(__real_write(fd, buf, 1) == 1);
			hang_helper();
		}
		/* Neither part alone exceeds the one-second timeout. */
		if (reply_behavior == REPLY_SLOW_PARTS)
			usleep(600000);
	}
...
int __wrap_close(int fd)
{
	...
	if (getpid() != parent_pid && fd == reply_fd && reply_behavior == REPLY_NO_EOF)
		hang_helper();
	ret = __real_close(fd);
	if (getpid() != parent_pid && fd == reply_fd && reply_behavior == REPLY_DELAYED_EXIT)
		hang_helper();
	return ret;
}
...
static void check_enumeration_timeouts(void)
{
	sigset_t saved_mask, mask;
	...
	for (blocked = 0; blocked < 2; blocked++) {
		...
		for (behavior = REPLY_NORMAL; behavior <= REPLY_SLOW_PARTS; behavior++) {
			struct cuda_device_map map = {};
			int64_t start = monotonic_ms();
			int ret;

			reply_behavior = behavior;
			if (behavior == REPLY_NORMAL)
				assert(setenv("CRIU_CUDA_MOCK_INIT_HANG", "1", 1) == 0);
			/* Exercise both dump and restore callers of the same helper. */
			ret = blocked ? cuda_gpu_inventory_dump() : cuda_device_map_resolve("auto", &map);
			assert(ret == -ETIMEDOUT);
			assert(monotonic_ms() - start < 5000);
			assert(!map.pairs && !map.count && !map.cli_value);
			if (behavior == REPLY_NORMAL)
		...
		reply_behavior = REPLY_NORMAL;
		cuda_plugin_timeout = 0;
		assert(setenv("CRIU_CUDA_MOCK_INIT_DELAY_MS", "1200", 1) == 0);
		check_automatic_map();
		assert(unsetenv("CRIU_CUDA_MOCK_INIT_DELAY_MS") == 0);
		check_signal_mask(&mask);
		errno = 0;
		assert(waitpid(-1, NULL, WNOHANG) == -1 && errno == ECHILD);
	}
